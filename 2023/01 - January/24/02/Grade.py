...
def getMode(data):
	frequency = {}
	# For Each Item In Data, Calculate It's Frequency
	for item in data:
	...
	# Find The Maximum Occurred Item
	max = data[0]
...
def main():
	...
	for fName in glob.glob(f'{dirName}/*.csv'):
		...
		# Map Function f(x) = (f / max_points) * 100 Over Points List To Get Percent Score
		data['Percent_Score'] = list(map(lambda x: (x / data['Max_Points'] * 100), data['Points']))
		
		# Map Function getGradeLetter Over Percent Score To Get Letter Grades
		data['Grades'] = list(map(getGradeLetter, data['Percent_Score']))
		
		# Get Mean, Median, Mode, Std Deviation Of Percent Score
		...
		# Sort Student Data By (Last Name + First Name)
		# First We Will Create A List Of Tuples Of By Zipping Lists Of Last Names,
		# First Name, Percent Scores, And Grade Letters:
		# [(Last_Name, First_Name, Percent_Score, Grade_Letter), ...]
		# Now Sort This Tuple List Using Key = (Last Name + First Name)
